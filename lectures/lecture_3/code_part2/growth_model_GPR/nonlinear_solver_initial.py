...
def initial(k_init, n_agents):
    ...
    print(f"nvars : {nvars}, X_L : {X_L} , X_U : {X_U}, M : {M}, G_L : {G_L}, G_U : {G_U}, NELE_JAC : {NELE_JAC}, NELE_HESS : {NELE_HESS}, eval_f : {eval_f}, eval_grad_f : {eval_grad_f}, eval_g : {eval_g}, eval_jac_g: {eval_jac_g}")
    print(f"{type(NELE_HESS)}")
    
    nlp = pyipopt.create(nvars, X_L, X_U, M, G_L, G_U, NELE_JAC, int(NELE_HESS), eval_f, eval_grad_f, eval_g, eval_jac_g)
